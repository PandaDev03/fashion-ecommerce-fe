  ...
  const columns: ColumnType<IOrder['items'][number]>[] = [
    {
      ...
      render: (_, record) => {
        return (
          <p>
            {record?.productName} - {record?.variantAttributes} *{' '}
            {record?.quantity}
          </p>
        );
      },
    ...
    <Layout loading={isGetOrderByNumberPending}>
      ...
      <Content className="w-full mx-auto max-w-[1920px] px-4 md:px-8 2xl:px-16">
        <div className="xl:px-32 2xl:px-44 3xl:px-56 py-16 lg:py-20">
          <div className="border border-gray-300 bg-gray-50 px-4 lg:px-5 py-4 rounded-md flex items-center justify-start text-primary text-sm md:text-base mb-6 lg:mb-8">
            <span className="w-10 h-10 ltr:mr-3 rtl:ml-3 ltr:xl:mr-4 rtl:xl:ml-4 rounded-full bg-gray-300 flex items-center justify-center shrink-0">
              <SuccessFill />
            </span>
            <span>Cảm ơn. Đơn đặt hàng của bạn đã được nhận.</span>
          </div>
          <div className="border border-gray-300 bg-gray-50 rounded-md flex flex-col md:flex-row mb-7 lg:mb-8 xl:mb-10">
            <div className="font-semibold text-primary text-base lg:text-lg border-b md:border-b-0 md:border-r border-dashed border-gray-300 px-4 lg:px-6 xl:px-8 py-4 md:py-5 lg:py-6 last:border-0">
